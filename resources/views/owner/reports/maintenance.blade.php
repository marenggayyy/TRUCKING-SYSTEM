            ...
            <div class="card-body p-3">
                <div class="table-responsive table-card-wrap">
                    <table class="table table-bordered mb-0">
                        ...
                        <tbody>
                            @forelse($personalVehicles ?? [] as $car)
                                <tr>
                                    <td class="fw-bold">{{ $car->plate_number }}</td>

                                    <td class="text-center">
                                        @include('partials.doc-status', ['doc' => $car->orcr])
                                    </td>

                                    <td class="text-center">
                                        @include('partials.doc-status', ['doc' => $car->insurance])
                                    </td>
                                    ...
                                    <td class="text-center">
                                        @include('partials.doc-status', ['doc' => $car->pms])
                                    </td>
                                    ...
                                    <td class="text-center">
                                        <div class="d-flex justify-content-center gap-1">
                                            ...
                                            <button class="btn btn-sm btn-primary personal-edit-btn"
                                                ...
                                                data-id="{{ $car->id }}" data-plate="{{ $car->plate_number }}"
                                                data-orcr="{{ optional($car->orcr)->expiry_date }}"
                                                data-insurance="{{ optional($car->insurance)->expiry_date }}"
                                                data-pms="{{ optional($car->pms)->expiry_date }}"
                                                data-orcr-file="{{ optional($car->orcr)->file_path }}"
                                                data-insurance-file="{{ optional($car->insurance)->file_path }}"
                                                data-pms-file="{{ optional($car->pms)->file_path }}">
                                                ✏
                                            </button>

                                            {{-- DELETE --}}
                                            <form action="{{ route('owner.personal-vehicles.delete', $car->id) }}"
                                                method="POST"
    ...
    <div class="modal fade" id="personalModal" tabindex="-1">
        <div class="modal-dialog modal-lg modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <form method="POST" action="{{ route('owner.personal-vehicle-docs.save') }}"
                    ...
                    <div class="modal-body">
                        ...
                        @foreach (['ORCR', 'INSURANCE', 'PMS'] as $type)
                            ...
                            <div id="{{ strtolower($type) }}PersonalPreview" class="mt-2"></div>

                            <div class="row mb-3">
                                <div class="col-md-6">
                                    <label>Expiry Date</label>
                                    <input type="date" name="{{ $type }}_expiry"
                                        id="{{ strtolower($type) }}PersonalInput" class="form-control">

                                    <div class="form-check mt-1">
                                        <input type="checkbox" name="delete_{{ $type }}_expiry" value="1"
                                            class="form-check-input">
                                ...
                                <div class="col-md-6">
                                    <label>Upload File</label>
                                    <input type="file" name="{{ $type }}_file" class="form-control">

                                    <div class="form-check mt-1">
                                        <input type="checkbox" name="delete_{{ $type }}_file" value="1"
                                            class="form-check-input">
                                        <label class="form-check-label text-danger">
                                            Delete File
                                        </label>
    ...
    <div class="modal fade" id="addPersonalVehicleModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form method="POST" action="{{ route('owner.personal-vehicles.store') }}">
                    @csrf
