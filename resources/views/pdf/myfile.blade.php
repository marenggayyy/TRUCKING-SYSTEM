...
<body>
    ...
    <div class="header">
        ...
        <div class="company">Payroll Period: {{ $weekStart }} - {{ $weekEnd }}</div>
    </div>
    ...
    <table class="main-table">
        ...
        @foreach ($p['rows'] as $r)
            <tr>
                ...
                <td>{{ number_format($r['total_salary'] ?? 0, 2) }}</td>
            </tr>
        @endforeach

        <tr>
            <td colspan="5"><strong>TOTAL INCOME</strong></td>
            <td><strong>₱ {{ number_format($p['payroll_total'], 2) }}</strong></td>
        </tr>
    </table>
    ...
    <table class="main-table">
        ...
        <tr>
            <td>{{ number_format($deductions['advanced'], 2) }}</td>
