                            ...
                            <div class="card iq-purchase" data-iq-gsap="onStart" data-iq-position-y="250"
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="d-flex align-items-center justify-content-between">
                                        ...
                                        <p class="mb-0 ms-2">Avg: ₱{{ number_format($destinationsStats['avg_rate'], 2) }}</p>
                        ...
                        <div class="card-group border rounded-1">
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <h5 class="mb-1">₱{{ number_format($todayData['gains'] ?? 0, 2) }}</h5>
                            ...
                            <div class="col">
                                <div class="card border h-100">
                                    <div class="card-body">
                                        ...
                                        <p class="mb-1 small">
                                            <strong>Destination:</strong> {{ $trip->destination?->store_name ?? 'N/A' }}
                                        </p>
                            ...
                            <div class="col">
                                <div class="card border h-100">
                                    <div class="card-body">
                                        ...
                                        <p class="mb-1 small"><strong>Code:</strong> {{ $destination->store_code ?? '—' }}</p>
                                        <p class="mb-2 small"><strong>Area:</strong> {{ $destination->area ?? '—' }}</p>

                                        @if(!empty($destination->remarks))
                                            <p class="mb-3 small text-muted fst-italic">{{ $destination->remarks }}</p>
                                        @endif

                                        <div class="d-flex justify-content-between align-items-center pt-2 border-top">
                                            <span class="small text-muted">Rate</span>
                                            <span class="fw-bold text-success">₱{{ number_format($destination->rate ?? 0, 2) }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <div class="col-lg-4">
        <div class="d-flex flex-column gap-3">
                ...
                <div class="card-body">
                    ...
                    <div class="row g-3">
                        <div class="col-6">
                            ...
                            <h5 class="fw-bolder text-white mb-0">₱{{ number_format($financialData['gains'] ?? 0, 2) }}</h5>
                        </div>
                        <div class="col-6">
                            <p class="mb-1 small text-white-50">Expenses</p>
                            <h5 class="fw-bolder text-danger mb-0">₱{{ number_format($financialData['expenses'] ?? 0, 2) }}</h5>
                        </div>
                        <div class="col-12">
                            <hr class="border-white-50 my-2">
                            <p class="mb-1 small text-white-50">Net Profit</p>
                            <h4 class="fw-bolder text-success mb-0">₱{{ number_format($financialData['profit'] ?? 0, 2) }}</h4>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card" data-iq-gsap="onStart" data-iq-position-y="70" data-iq-rotate="0"
                ...
                <div class="card-body">
                    @forelse($recentTrips as $trip)
                        <div class="d-flex justify-content-between align-items-center flex-wrap py-3 @if(!$loop->last) border-bottom @endif">
                            <div class="flex-grow-1">
                                <h6 class="mb-0">{{ $trip->trip_ticket_no ?? 'N/A' }}</h6>
                                <span class="text-muted small">{{ $trip->destination?->store_name ?? 'No destination' }}</span>
                            </div>

                            <div class="text-end">
                                <span class="badge bg-@if(($trip->status ?? '') === 'Completed') success @elseif(($trip->status ?? '') === 'Dispatched') warning @else secondary @endif">
                                    {{ $trip->status ?? '—' }}
                                </span>
            ...
            <div class="card iq-purchase" data-iq-gsap="onStart" data-iq-position-y="70" data-iq-rotate="0" data-iq-trigger="scroll" data-iq-ease="power.out" data-iq-opacity="0">
                ...
                <div class="card-body">
                    @php $audits = \App\Models\AuditTrail::with('user')->latest()->limit(10)->get(); @endphp
                    @forelse($audits as $audit)
                        <div class="py-3 @if(!$loop->last) border-bottom @endif">
                            <div class="d-flex justify-content-between align-items-start mb-2">
                                <div>
                                    <h6 class="mb-1">{{ $audit->user->name ?? 'System' }}</h6>
